function fname = print_table_DM_comb(DM, T,model, p_bar, methods,v)

    fname = ['results/',model,'/',model,'_DM',v,'_T',num2str(T),'.tex'];
...
%         ' & \\multicolumn{1}{c}{PCP10\\%%} 
    ...
    for pp = 1:3    
        ...
        alpha = 0.10;
        SIG = logical(abs(DM(:,:,pp)) > norminv(1-alpha/2)); % 1.6449
        STAR(SIG) = {'$^{*}$\phantom{$^{**}$}'};

        alpha = 0.05;
        SIG = logical(abs(DM(:,:,pp)) > norminv(1-alpha/2)); % 1.9600
        STAR(SIG) = {'$^{**}$\phantom{$^{*}$}'};

        alpha = 0.01;
        SIG = logical(abs(DM(:,:,pp)) > norminv(1-alpha/2)); % 2.5758
        STAR(SIG) = {'$^{***}$'};


        fprintf(FID, ['\\multicolumn{',num2str(met+1),'}{c}{$\\tau = ',num2str(p_bar(pp)),'$} \\\\ \\hline \n']);
        ...
        for ii = 1:met
            ...
            for jj = 1:met
                if (jj == met)
                    fprintf(FID,'%6.4f%s  ',DM(ii,jj,pp),char(STAR{ii,jj})); 
                else
                    fprintf(FID,'%6.4f%s & ',DM(ii,jj,pp),char(STAR{ii,jj})); 
                end
            end
...
%     end
    ...
    label = ['\\label{tab:',model,'_DM_',v,'_T',num2str(T),'}  \n'];
    fprintf(FID, label);
