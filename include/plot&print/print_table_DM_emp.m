function fname = print_table_DM_emp(DM, data, p_bar, methods,v)

    fname = ['results/tgarch11/',data,'/',data,'_DM',v,'_',num2str(p_bar),'2.tex'];

    data = data(1:(strfind(data,'_')-1));
%     methods = {'True','Posterior','CP0','PCP0','CP10\%','PCP10\%'};
    ...
    fprintf(FID, ['\\begin{tabular}{l | ', repelem('r',met), '} \n']);
        ...
        for ii = 1:met
            fprintf(FID, ['& \\multicolumn{1}{c}{',methods{ii},'}']);
        end
...
%     fprintf(FID, ['\\multicolumn{',num2str(met+1),'}{l}{\\footnotesize{(P)CP$_{var}$: Censoring with time-varying threshold at 10\\%% MLE implied quantile.}} \\\\ \n']); 
    ...
    caption = ['\\caption{',data,...
        ...
        ' of forecasting performance in the $\\tau$\\%% tail based on ', ...
        ' the censored likelihood score rule with ', vvv, ' threshold for evaluation, ', ...
        ' for $H=1500$ out-of-sample periods, ', ...
    ...
    label = ['\\label{tab:',data,'_DM',v,'_',num2str(p_bar),'}  \n'];
    fprintf(FID, label);
