function print_table_pcp_mc(model,parameters,sigma1,sigma2)
    ...
    fprintf(FID, 'Value & & True/MC$^*$ & Posterior & CP  & PCP \\\\ \\hline \n'); 
    ...
    for T = [100,1000,10000]
        ...
        for ii = 1:length(parameters)
             param = parameters(ii);
             fprintf(FID,[char(param),'&& %6.4f & %6.4f & %6.4f & %6.4f  \\\\ \n'], param_true(ii), mean(draw(:,ii)), mean(draw_C(:,ii)), mean(draw_PC(:,ii)));        
        ...
        fprintf(FID,'  && [%6.4f] & [%6.4f] & [%6.4f] & [%6.4f]  \\\\ \n', ...
            mean(MSE_1), mean(MSE_1_post), mean(MSE_1_post_C),mean(MSE_1_post_PC));        
        fprintf(FID,'VaR 5\\%% && %6.4f & %6.4f & %6.4f & %6.4f  \\\\ \n', ...
            mean(VaR_5), mean(VaR_5_post), mean(VaR_5_post_C),mean(VaR_5_post_PC));               
        fprintf(FID,' && [%6.4f] & [%6.4f] & [%6.4f] & [%6.4f]  \\\\ \n', ...
            mean(MSE_5), mean(MSE_5_post), mean(MSE_5_post_C),mean(MSE_5_post_PC));        
    end
