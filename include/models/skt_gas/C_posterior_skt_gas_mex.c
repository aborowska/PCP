...
void C_posterior_skt_gas_mex(double *theta, double *y, double *threshold, 
...
//                 mexPrintf("z[%i]=%16.14f\n",0,z);
            ...
            else //cdf
            {
                i2 = i2+1;
                // stationary distribution for the first observation
                z2[i2] = (*threshold - theta[i+2*N])/scale;                      
            }
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    double *theta, *y, *y_S, *threshold, *hyper;   /* input*/
//     double *d, *h;                                 /* output */
    double *d;                                 /* output */
    ...
    y_S = mxGetPr(prhs[4]); /* hyperparameter on degrees of freedom*/
...
//     h = mxGetPr(plhs[2]);
    ...
    /* call the function */
    C_posterior_skt_gas_mex(theta, y, threshold, N, T, hyper, y_S, d); 
    T_out[0] = T;
}
