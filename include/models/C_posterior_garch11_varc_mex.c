...
void C_posterior_garch11_varc_mex(double *theta, double *y, double *threshold, double *y_S,
        mwSignedIndex N, mwSignedIndex T, double *d, double *h)
{
    ...
    {           
        if (r1[i]==1) // compute only for valid draws
        {
            ...
            if (y[0] < threshold[0])  //pdf   // <==== CHECK THAT
            {
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    /* call the function */
    C_posterior_garch11_varc_mex(theta, y, threshold, y_S, N, T, d, h); 
    T_out[0] = T;
}