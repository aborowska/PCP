...
%                 [mit, CV] = MitISEM_new(mit, kernel, mu_init, cont, GamMat);            
    ...
    if arg2
        ...
        threshold = threshold(round(0.1*T));
        ...
        [mu_C,~,~,~,~,Sigma_C2] = fminunc(kernel_init,mu_init,options);
    ...
    %     cont.mit.CV_tol = 0.3; 
        ...
        ind_ok = (draw_C(:,4)>0);
        % predictive densities
        dens_C = predictive_dens_t_gas(y(T:(T+H)), fT_C(ind_ok), draw_C(ind_ok,:));
        % predicitve cdfs, constant threshold for different tails
        cdf_C = predictive_cdf_t_gas(y(T:(T+H)), fT_C(ind_ok,:), draw_C(ind_ok,:), THR_emp);       
        C_score_C = C_ScoringRule(dens_C, cdf_C, y((T+1):(T+H)), THR_emp);  
        % predicitve cdfs, var mle threshold for different tails
        cdf_v_C = predictive_cdf_t_gas(y(T:(T+H)), fT_C(ind_ok,:), draw_C(ind_ok,:), THR_MLE);       
...
%% CP and PCP with 20% threshold 
    if arg20     
        PCP_t_gas_empirical_diff_threshold;
    end
