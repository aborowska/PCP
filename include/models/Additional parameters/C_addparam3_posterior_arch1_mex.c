...
void C_addparam3_posterior_arch1_mex(double *theta_add, double *theta, double *y, double *threshold, double *y_S,
...
//                 sigma = mu/(theta_add[i+1*N]*h[i]);
                sigma = sigma + log(theta_add[i+2*N]*h[i]); 
...
//                 sigma = theta_add[i+1*N]*h[i];
            ...
            {   
                ...
                h[i] = theta[i+N]*(1.0-theta[i+3*N]) + theta[i+3*N]*(mu*mu);
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    /* call the function */
    C_addparam3_posterior_arch1_mex(theta_add, theta, y, threshold, y_S, N, T, d, h); 
    T_out[0] = T;
}