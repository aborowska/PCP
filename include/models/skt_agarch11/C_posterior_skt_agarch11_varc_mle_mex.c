...
void C_posterior_skt_agarch11_varc_mle_mex(double *theta, double *y, 
...
*/       
    ...
    double a, b, c, tmp;
    double sum_cdf, *cdf, logpdf, h_mle, *THR;
    mwSignedIndex i, j;
...
//     }    
    ...
    { 
        ...
        {
            mu = y[j-1] - mu_mle[4];
            h_mle = mu_mle[3]*(1.0-mu_mle[5]-mu_mle[6]) + mu_mle[5]*(mu*mu) + mu_mle[6]*h_mle;     
        }       
        THR[j] = (y[j] - mu_mle[2])/sqrt(h_mle);
        ...
        THR[j] = mu_mle[2] + sqrt(h_mle)*threshold[0];
    }
...
*/       
    ...
    /* call the function */
    C_posterior_skt_agarch11_varc_mle_mex(theta, y, mu_mle, threshold,
            y_S, N, T, hyper, d, h); 
    T_out[0] = T;
}