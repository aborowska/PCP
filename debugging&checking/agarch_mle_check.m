% AGARCH: ML estimation MSE (unbiasedness)
...
MUS_C_y_S = zeros(S,5);
% MUS_C_omega = zeros(S,5);

MUS_C0_y_S = zeros(S,5);
% MUS_C0_omega = zeros(S,5);
...
for ss = 1:S
    ...
    if TRANS
        ...
        [mu,~,~,~,~,Sigma] = fminunc(kernel_init, transform_param_agarch(mu_init, 'opt'),options);
        Sigma = inv(T*Sigma);
...
%     MUS_omega(ss,:) = mu;
    ...
    if TRANS
        kernel_init = @(xx) - C_posterior_agarch11_mex(transform_param_agarch(xx, 'back'), y(1:T,1), threshold, y_S)/T;    
        [mu_C,~,~,~,~,Sigma_C] = fminunc(kernel_init, transform_param_agarch(mu_init, 'opt'),options);
        Sigma_C = inv(T*Sigma_C);
        MUS_C_y_S(ss,:) = transform_param_agarch(mu_C, 'back'); 
    else    
...
%     kernel_init = @(xx) - C_posterior_agarch11_mex(xx, y(1:T,1), threshold, 0)/T;    
%     [mu_C,~,~,~,~,Sigma_C] = fminunc(kernel_init,mu_init,options);
%     Sigma_C = inv(T*Sigma_C);
%     MUS_C_omega(ss,:) = mu_C; 
    ...
    threshold0 = 0;
    if TRANS
        kernel_init = @(xx) - C_posterior_agarch11_mex(transform_param_agarch(xx, 'back'), y, threshold0, y_S)/T;    
        [mu_C0,~,~,~,~,Sigma_C0] = fminunc(kernel_init, transform_param_agarch(mu_init, 'opt'),options);
        Sigma_C0 = inv(T*Sigma_C0);
        MUS_C0_y_S(ss,:) = transform_param_agarch(mu_C0, 'back');
    else 
        kernel_init = @(xx) - C_posterior_agarch11_mex(xx, y, threshold0, y_S)/T;    
        [mu_C0,~,~,~,~,Sigma_C0] = fminunc(kernel_init, mu_init,options);
        Sigma_C0 = inv(T*Sigma_C0);
        MUS_C0_y_S(ss,:) = mu_C0;     
    end
%     kernel_init = @(xx) - C_posterior_agarch11_mex(xx, y, threshold0, 0)/T;    
%     [mu_C0,~,~,~,~,Sigma_C0] = fminunc(kernel_init,mu_init,options);
%     Sigma_C0 = inv(T*Sigma_C0);
