function results = PCP_arch1_run_add(sdd, c, sigma1, sigma2, kappa, omega, alpha, p_bar1, p_bar, T, H, M, BurnIn, mu_init, df, cont, options, partition, II, GamMat)
...
%     y_post_add = bsxfun(@plus,sqrt(h_add).*randn(M,H),mu_add(:,1) + mu_add(:,2).*draw(:,1));
    ...
    kernel = @(xx) C_posterior_arch1_mex(xx, y(1:T,1), threshold, y_S);
...
%     cont.mit.CV_tol = 0.3; 
    ...
    kernel_init = @(xx) - C_posterior_arch1_mex(xx, y(1:T,1), threshold0, y_S)/T;    
    kernel = @(xx) C_posterior_arch1_mex(xx, y(1:T,1), threshold0, y_S);
    ...
    h_post_C0 = volatility_arch1(draw_C0,y,y_S,H);
...
%     [draw_PC0, a_PC0] = sim_cond_mit_MH(mit_C0, draw_short, partition, M_short, BurnIn, kernel, GamMat);
    ...
    h_post_PC0 = volatility_arch1(draw_PC0,y,y_S,H);
